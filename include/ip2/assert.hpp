...
extern TORRENT_EXPORT char const* ip2_assert_log;
#endif
...
#define TORRENT_ASSERT_PRECOND(x) \
	do { if (x) {} else ip2::assert_fail(#x, __LINE__, __FILE__, __func__, nullptr, 1); } TORRENT_WHILE_0

#define TORRENT_ASSERT(x) \
	do { if (x) {} else ip2::assert_fail(#x, __LINE__, __FILE__, __func__, nullptr, 0); } TORRENT_WHILE_0

#if TORRENT_USE_IOSTREAM
#define TORRENT_ASSERT_VAL(x, y) \
	do { if (x) {} else { std::stringstream __s__; __s__ << #y ": " << y; \
	ip2::assert_fail(#x, __LINE__, __FILE__, __func__, __s__.str().c_str(), 0); } } TORRENT_WHILE_0

#define TORRENT_ASSERT_FAIL_VAL(y) \
	do { std::stringstream __s__; __s__ << #y ": " << y; \
	ip2::assert_fail("<unconditional>", __LINE__, __FILE__, __func__, __s__.str().c_str(), 0); } TORRENT_WHILE_0
...
#define TORRENT_ASSERT_FAIL() \
	ip2::assert_fail("<unconditional>", __LINE__, __FILE__, __func__, nullptr, 0)
