...
namespace errors {
	// ip2 uses boost.system's ``error_code`` class to represent
	// errors. ip2 has its own error category
	// ip2_category() with the error codes defined by
	// error_code_enum.
	enum error_code_enum
	{
		...
		// The .torrent file has more pieces than is supported by ip2
	...
	// HTTP errors are reported in the ip2::http_category, with error code enums in
	// the ``ip2::errors`` namespace.
	enum http_errors
	{
		cont = 100,
		ok = 200,
	...
	// return the instance of the ip2_error_category which
	// maps ip2 error codes to human readable error messages.
	TORRENT_EXPORT boost::system::error_category& ip2_category();
...
#if TORRENT_ABI_VERSION == 1
	TORRENT_DEPRECATED
	inline boost::system::error_category& get_ip2_category()
	{ return ip2_category(); }
...
namespace system {
	...
	template<> struct is_error_code_enum<ip2::errors::error_code_enum>
	{ static const bool value = true; };

	template<> struct is_error_code_enum<ip2::errors::http_errors>
	{ static const bool value = true; };
}
}
