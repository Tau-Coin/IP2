...
#include "ip2/operations.hpp" // for operation_t enum
...
#include "ip2/aux_/noexcept_movable.hpp"
#include "ip2/portmap.hpp" // for portmap_transport
#include "ip2/socket_type.hpp"
#include "ip2/client_data.hpp"
#include "ip2/aux_/deprecated.hpp"
...
#define TORRENT_DEFINE_ALERT_PRIO(name, seq, prio) \
	...
	// Whenever ip2 learns about the machines external IP, this alert is
	...
	// enum_if - tcp
	//
	// ip2 may sometimes try to listen on port 0, if all other ports failed.
	...
	struct TORRENT_EXPORT listen_failed_alert final : alert
	{
		...
		// the network device ip2 attempted to listen on, or the IP address
	...
	struct TORRENT_EXPORT listen_succeeded_alert final : alert
	{
		...
		// the address ip2 ended up listening on. This address
		// refers to the local interface.
		aux::noexcept_movable<lt::address> address;

		// the port ip2 ended up listening on.
		int const port;
	...
	struct TORRENT_EXPORT session_stats_alert final : alert
	{
		...
		// for a specific version of ip2, but may differ for other
	...
	struct TORRENT_EXPORT dht_pkt_alert final : alert
	{
		...
		// is valid, which is owned by ip2 and reclaimed whenever
    ...
    {
        // internal
        TORRENT_UNEXPORT blockchain_new_head_block_alert(aux::stack_allocator& alloc, ip2::blockchain::block blk);
    ...
    {
        // internal
        TORRENT_UNEXPORT blockchain_new_tail_block_alert(aux::stack_allocator& alloc, ip2::blockchain::block blk);
    ...
    {
        // internal
        TORRENT_UNEXPORT blockchain_new_consensus_point_block_alert(aux::stack_allocator& alloc, ip2::blockchain::block blk);
    ...
    {
        // internal
        TORRENT_UNEXPORT blockchain_rollback_block_alert(aux::stack_allocator& alloc, ip2::blockchain::block blk);
    ...
    {
        // internal
        TORRENT_UNEXPORT blockchain_top_three_votes_alert(aux::stack_allocator& alloc, aux::bytes id, std::vector<ip2::blockchain::vote> vs);
        ...
        // top three votes
        std::vector<ip2::blockchain::vote> votes;
    };
    ...
    {
        // internal
        TORRENT_UNEXPORT blockchain_new_transaction_alert(aux::stack_allocator& alloc, ip2::blockchain::transaction t);
    ...
    {
        // internal
        TORRENT_UNEXPORT blockchain_state_alert(aux::stack_allocator& alloc, aux::bytes id, ip2::blockchain::account t);
    ...
    {
        // internal
        TORRENT_UNEXPORT blockchain_state_array_alert(aux::stack_allocator& alloc, aux::bytes id, std::vector<ip2::blockchain::account> acts);
        ...
        // state from peers.
        std::vector<ip2::blockchain::account> accounts;
    };
