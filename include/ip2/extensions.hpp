...
#include "ip2/peer_info.hpp" // for peer_source_flags_t

// OVERVIEW
//
// ip2 has a plugin interface for implementing extensions to the protocol.
...
// structures it is also quite easy to sabotage ip2's operation.
//
// All the callbacks are always called from the ip2 network thread. In
// case portions of your plugin are called from other threads, typically the main
// thread, you cannot use any of the member functions on the internal structures
// in ip2, since those require being called from the ip2 network
...
// ip2 thread (through a callback). If you need to send out a message
...
// specific version of ip2. Although, in practice the internals mostly
...
// They are found in the ``<ip2/extensions.hpp>`` header.
//
...
// -------------
//
// Since plugins are running within internal ip2 threads, one convenient
...
// not collide with any other alert. The built-in alerts in ip2 will
...
namespace ip2 {
	...
	struct TORRENT_EXPORT plugin
	{
		...
		using feature_flags_t = ip2::feature_flags_t;
#endif
	...
	// ability to change or override the default ip2 behavior.
...
#if TORRENT_ABI_VERSION == 1
		using flags_t = ip2::add_peer_flags_t;
#endif
	...
	struct TORRENT_EXPORT peer_plugin
	{
		...
		// called when ip2 think this peer should be disconnected.
