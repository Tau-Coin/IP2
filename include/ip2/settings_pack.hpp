...
namespace aux {
	...
	struct TORRENT_EXPORT settings_pack final : settings_interface
	{
		...
		enum string_types
		{
			...
			// ip2/ip2-version". This name will not only be used when
		...
		enum bool_types
		{
			...
			// ``close_redundant_connections`` specifies whether ip2 should
			...
			// has been paused by the auto managed feature in ip2, i.e. the
			...
			// of redundant bytes. If this is ``false``, ip2 attempts to
			...
			// connections are disabled, ip2 will simply not make
			...
			// when this is true, ip2 will not attempt to make outgoing
			...
			// ip2 is downloading to or seeding from. This is implemented
			// using ``fcntl(F_SETLK)`` on Unix systems and by not passing in
			// ``SHARE_READ`` and ``SHARE_WRITE`` on windows. This might prevent
			// 3rd party processes from corrupting the files under ip2's
			// feet.
...
#if TORRENT_ABI_VERSION == 1
			// ``contiguous_recv_buffer`` determines whether or not ip2
			...
			// if false, prevents ip2 to advertise share-mode support
			support_share_mode,
			...
			// if this is true, ip2 will fall back to listening on a port
		...
		enum int_types
		{
			...
			// ``suggest_mode`` controls whether or not ip2 will send out
			...
			//   high throughput and large files. If ip2's read cache is
			...
			// write errors occur on a torrent until ip2 will take it out
			// of the upload mode, to test if the error condition has been fixed.
			//
			// ip2 will only do this automatically for auto managed
			// torrents.
			//
			...
			// messages, this limit prevents ip2 from using too much RAM.
			max_suggest_pieces,
			...
			// ``udp_tracker_token_expiry`` is the number of seconds ip2
			...
			// bandwidth. 0 means automatic, where ip2 opens up 20% of your
			...
			// connections ip2 will have when connecting to peers. A
			...
			//
			// This setting may be useful to make ip2 more or less
			...
			// and then to the downloading torrent. ip2 will loop over the
			...
			// ip2 from creating too many expensive http requests per
			// second. You can select a value as high as you want but keep in mind
			// that ip2 can't create parallel requests if the first request
