...
// alerts (warnings, messages and errors from ip2). If no alerts have
// been posted by ip2 pop_alerts() will return an empty list.
//
...
namespace alert_category {
	...
	// Enables debug logging alerts. These are available unless ip2
	...
	// alerts on transport log
	inline constexpr alert_category_t transport_log = 27_bit;

	// alerts on assemble log
	inline constexpr alert_category_t assemble_log = 28_bit;

	// alerts on events in assemble
	inline constexpr alert_category_t assemble = 29_bit;
...
#include "ip2/aux_/disable_deprecation_warnings_push.hpp"
	...
	// pointers returned by ip2 are short lived (the details are described
...
#include "ip2/aux_/disable_warnings_pop.hpp"
		...
		static inline constexpr alert_category_t transport_log_notification = 27_bit;
		static inline constexpr alert_category_t assemble_log_notification = 28_bit;
		static inline constexpr alert_category_t assemble_notification = 29_bit;
