...
#include "ip2/socks5_stream.hpp" // for socks_error
#include "ip2/aux_/keepalive.hpp"
...
struct set_dont_frag
{
		...
		, m_df(df)
	{
		...
		m_socket.set_option(ip2::dont_fragment(true), ignore_errors);
	...
	~set_dont_frag()
	{
		if (!m_df) return;
		error_code ignore_errors;
		m_socket.set_option(ip2::dont_fragment(false), ignore_errors);
		TORRENT_ASSERT_VAL(!ignore_errors, ignore_errors.message());
	}
...
#error BOOST_ASIO_ENABLE_CANCELIO needs to be defined when building ip2 to enable cancel() in asio on windows
#endif
