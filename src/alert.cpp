...
namespace {
	...
	transport_log_alert::transport_log_alert(aux::stack_allocator& alloc, char const* log)
			: m_alloc(alloc)
			, m_str_idx(alloc.copy_string(log))
	{}

	transport_log_alert::transport_log_alert(aux::stack_allocator& alloc, char const* fmt, va_list v)
			: m_alloc(alloc)
			, m_str_idx(alloc.format_string(fmt, v))
	{}

	char const* transport_log_alert::log_message() const
	{
		return m_alloc.get().ptr(m_str_idx);
	}

#if TORRENT_ABI_VERSION == 1
	char const* transport_log_alert::msg() const
	{
		return log_message();
	}
#endif   

	std::string transport_log_alert::message() const
	{
#ifdef TORRENT_DISABLE_ALERT_MSG
		return {};
#else
