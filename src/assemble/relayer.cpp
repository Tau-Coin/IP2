...
std::tuple<sha256_hash, api::error_code> relayer::relay_message(
	...
	, span<char const> message)
{
	...
	if (m_session.dht_nodes() == 0)
	{
		...
		m_logger.log(aux::LOG_ERR
			, "drop relay req:%s, error: dht nodes 0", hex_key);
#endif
...
#ifndef TORRENT_DISABLE_LOGGING
		m_logger.log(aux::LOG_ERR
			, "drop relay req:%s, error: buffer is full", hex_key);
#endif
...
#ifndef TORRENT_DISABLE_LOGGING
		m_logger.log(aux::LOG_ERR
			, "drop relay req:%s, too large:%d", hex_key, (int)message.size());
#endif
	...
	api::error_code ok = m_session.transporter()->send(receiver, pl
		, std::bind(&relayer::send_callback, self(), _1, _2, ctx)
...
void relayer::send_callback(entry const& payload
	...
	, std::shared_ptr<relay_context> ctx)
{
	...
	else if (ctx->get_relay_type() == URI)
	{
		// TODO: post 'relay_uri_alert'
	}
