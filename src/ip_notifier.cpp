...
struct ip_change_notifier_impl final : ip_change_notifier
{
			...
			, netlink::endpoint(netlink(NETLINK_ROUTE), RTMGRP_IPV4_IFADDR))
	{
		...
		error_code ec;
		m_socket.set_option(ip2::no_enobufs(true), ec);
	}
...
struct ip_change_notifier_impl final : ip_change_notifier
{
	explicit ip_change_notifier_impl(io_context& ios)
		: m_ios(ios)
	{
		m_queue = dispatch_queue_create("ip2.IPChangeNotifierQueue", nullptr);
...
#pragma clang diagnostic ignored "-Wold-style-cast"
	...
	CFRef<SCDynamicStoreRef> store{SCDynamicStoreCreate(nullptr
		, CFSTR("ip2.IPChangeNotifierStore"), callback, &context)};
#if defined __clang__
