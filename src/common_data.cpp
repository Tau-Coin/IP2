...
bool ip2::aux::isHex(string const& _s) noexcept
{
	auto it = _s.begin();
...
std::string ip2::aux::escaped(std::string const& _s, bool _all)
{
...
bytes ip2::aux::fromHex(std::string const& _s, WhenError _throw)
{
...
bytes ip2::aux::asNibbles(bytesConstRef const& _s)
{
	bytes ret;
	ret.reserve(_s.size() * 2);
	for (auto i: _s)
